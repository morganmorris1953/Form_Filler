...
def Write_AF_form_911(excelFileInfo, referencePath, selectedItems, excelColumnToSearch):
    form911Path = os.path.join(referencePath, 'main_af-form-911-enlisted-performance-report-msgt-thru-smsgt.pdf')
    delayTime = 3
    interval_time = 0.0
    pdfCommand = 'start ' + form911Path
    ...
    while (excelFileInfo.cell(row = i, column = excelColumnToSearch)).value != None:
        for item in selectedItems:
            if item == (excelFileInfo.cell(row = i, column = excelColumnToSearch)).value:
                ...
                pyautogui.press('space', interval=interval_time)         
                pyautogui.press('tab', presses=7, interval=interval_time)
                pyautogui.press('space', interval=interval_time) 
